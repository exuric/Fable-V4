...
run(function()
	...
	local function updateStore(newStore, oldStore)
		if newStore.GameCurrency ~= oldStore.GameCurrency then
			fableEvents.CurrencyChange:Fire(table.clone(newStore.GameCurrency.Quantities))
		end
	...
	fable:Clean(workspace.BlockContainer.DescendantAdded:Connect(function(v)
		parsePositions(v, function(pos)
			store.blocks[pos] = v
		end)
	end))
	fable:Clean(workspace.BlockContainer.DescendantRemoving:Connect(function(v)
		parsePositions(v, function(pos)
...
run(function()
	...
	InvMove = fable.Categories.Blatant:CreateModule({
		Name = 'InvMove',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local anims, armC0 = fable.Libraries.auraanims
		...
		Function = function(callback)
			if callback then
				...
				repeat
					...
					if Attacking and fable.ThreadFix then
						setthreadidentity(8)
					end
...
run(function()
	...
	AutoBuy = fable.Categories.Inventory:CreateModule({
		...
		Function = function(callback)
			if callback then
				AutoBuy:Clean(fableEvents.CurrencyChange.Event:Connect(buyCheck))
	...
	Viewmodel = fable.Legit:CreateModule({
		...
		Function = function(callback)
			if callback then 
				...
				fable:Clean(entitylib.Events.LocalAdded:Connect(newCharacter))
				if entitylib.isAlive then 
