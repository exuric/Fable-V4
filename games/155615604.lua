...
local function isTarget(plr)
	return (table.find(fable.Categories.Targets.ListEnabled, plr.Name) or tempTargets[plr.Name]) and true
end
...
run(function()
	function Cheats:Flag(plr, flagType, limit)
		...
		if flags[flagType] > limit then
			...
			fableEvents.CheatFlagged:Fire(plr, flagType)
		end
	end
...
run(function()
	...
	entitylib.getEntityColor = function(entity)
		if not (entity.Player and fable.Settings.Modules.Options['Use team color'].Enabled) then
			return
		end
...
run(function()
	...
	if not (pl.Bullet and pl.SwitchTable) then
		repeat
			getShootFunction()
			task.wait()
		until pl.Bullet and pl.SwitchTable or fable.Loaded == nil
	...
	for _, event in {'EntityAdded', 'LocalAdded'} do
		fable:Clean(entitylib.Events[event]:Connect(function()
			OriginScanner:UpdateIgnore()
		end))
	end

	fable:Clean(replicatedStorage.Killfeed.ChildAdded:Connect(function(obj)
		local names = {}
		...
		fableEvents.PlayerKill:Fire(unpack(names))
	...
	fable:Clean(fableEvents.Arrested.Event:Connect(function()
		arrests:Increment()
	end))

	fable:Clean(replicatedStorage.Remotes.MessageReceived.OnClientEvent:Connect(function(msg)
		if msg:find('kicked') then
			cheaterkicked:Increment()

			task.defer(function()
				fableEvents.CheaterKicked:Fire(msg:sub(1, msg:find(' ')))
			end)
		end
	end))
	...
	fable:Clean(entitylib.Events.EntityUpdated:Connect(function(entity)
		if entity.Player and entity.Player.Team == teams.Inmates then
			fable.Categories.Friends.ColorUpdate:Fire()
		end
	end))
...
run(function()
	...
	AntiInvisible = fable.Categories.Blatant:CreateModule({
		Name = 'AntiInvisible',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AntiKillPlane = fable.Categories.Blatant:CreateModule({
		Name = 'AntiKillPlane',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AntiRiotShield = fable.Categories.Blatant:CreateModule({
		Name = 'AntiRiotShield',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AntiTaze = fable.Categories.Blatant:CreateModule({
		Name = 'AntiTaze',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoReset = fable.Categories.Blatant:CreateModule({
		Name = 'AutoReset',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoTaser = fable.Categories.Blatant:CreateModule({
		Name = 'AutoTaser',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	FenceGodmode = fable.Categories.Blatant:CreateModule({
		Name = 'FenceGodmode',
		Function = function(callback)
...
run(function()
	...
	NoJumpCooldown = fable.Categories.Blatant:CreateModule({
		Name = 'NoJumpCooldown',
		Function = function(callback)
			if callback then
...
run(function()
	...
	VehicleFly = fable.Categories.Blatant:CreateModule({
		Name = 'VehicleFly',
		Function = function(callback)
			if callback then
				up, down = 0, 0
...
run(function()
	...
	VehicleSpeed = fable.Categories.Blatant:CreateModule({
		Name = 'VehicleSpeed',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	VehicleWallbang = fable.Categories.Blatant:CreateModule({
...
run(function()
	...
	C4ESP = fable.Categories.Render:CreateModule({
		Name = 'C4ESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	CameraPhase = fable.Categories.Render:CreateModule({
		Name = 'CameraPhase',
		Function = function(callback)
			if callback then
...
run(function()
	...
	KillNotifications = fable.Categories.Render:CreateModule({
		Name = 'KillNotifications',
		Function = function(callback)
			if callback then
				KillNotifications:Clean(fableEvents.PlayerKill.Event:Connect(function(killer, victim)
...
run(function()
	...
	AutoDetonate = fable.Categories.Utility:CreateModule({
		Name = 'AutoDetonate',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoReload = fable.Categories.Utility:CreateModule({
		Name = 'AutoReload',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoToxic = fable.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			if callback then
				AutoToxic:Clean(fableEvents.CheaterKicked.Event:Connect(function(plr)
...
run(function()
	...
	CheatDetector = fable.Categories.Utility:CreateModule({
		Name = 'CheatDetector',
		Function = function(callback)
			if callback then
				CheatDetector:Clean(fableEvents.CheatFlagged.Event:Connect(function(plr, flagType)
...
run(function()
	...
	AutoArmor = fable.Categories.Inventory:CreateModule({
		Name = 'AutoArmor',
		Function = function(callback)
			if callback then
...
run(function()
	...
	BulletTracers = fable.Legit:CreateModule({
		Name = 'BulletTracers',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Crosshair = fable.Legit:CreateModule({
		Name = 'Crosshair',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HitSound = fable.Legit:CreateModule({
		Name = 'HitSound',
		Function = function(callback)
			if callback then
				local played
...
run(function()
	...
	KillSound = fable.Legit:CreateModule({
		Name = 'KillSound',
		Function = function(callback)
			if callback then
				KillSound:Clean(fableEvents.PlayerKill.Event:Connect(function(plr)
