...
local kickThread = task.spawn(lplr.Kick, lplr, 'Bedwars is no longer supported by Fable V4, thank you for 5 years of support ❤️')
...
local getcustomasset = fable.Libraries.getcustomasset
...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('FableV4/assets/new/blur.png')
...
local function hotbarSwitch(slot)
	if slot and store.inventory.hotbarSlot ~= slot then
		...
		fableEvents.InventoryChanged.Event:Wait()
		return true
	end
	return false
end
			...
			if hum and humrootpart then
				...
				if plr == lplr then
					...
					table.insert(entitylib.Connections, char.AttributeChanged:Connect(function(attr)
						fableEvents.AttributeChanged:Fire(attr)
					end))
				else
...
run(function()
	...
	for i, v in remoteNames do
		...
		if remote == '' then
			notif('Fable', 'Failed to grab remote ('..i..')', 10, 'alert')
		end
		remotes[i] = remote
	end
	...
	local function updateStore(new, old)
		...
		if new.Inventory ~= old.Inventory then
			...
			if newinv ~= oldinv then
				fableEvents.InventoryChanged:Fire()
			end

			if newinv.inventory.items ~= oldinv.inventory.items then
				fableEvents.InventoryAmountChanged:Fire()
	...
	for _, event in {'MatchEndEvent', 'EntityDeathEvent', 'BedwarsBedBreak', 'BalloonPopped', 'AngelProgress', 'GrapplingHookFunctions'} do
		if not fable.Connections then return end
		bedwars.Client:WaitFor(event):andThen(function(connection)
			fable:Clean(connection:Connect(function(...)
				fableEvents[event]:Fire(...)
			end))
		end)
	end

	fable:Clean(bedwars.ZapNetworking.EntityDamageEventZap.On(function(...)
		fableEvents.EntityDamageEvent:Fire({
			entityInstance = ...,
			damage = select(2, ...),
			damageType = select(3, ...),
	...
	for _, event in {'PlaceBlockEvent', 'BreakBlockEvent'} do
		fable:Clean(bedwars.ZapNetworking[event..'Zap'].On(function(...)
			local data = {
				blockRef = {
					blockPosition = ...,
				},
	...
	task.spawn(function()
		pcall(function()
			repeat task.wait() until store.matchState ~= 0 or fable.Loaded == nil
	...
	fable:Clean(fableEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
		if bedTable.player and bedTable.player.UserId == lplr.UserId then
			beds:Increment()
		end
	end))

	fable:Clean(fableEvents.MatchEndEvent.Event:Connect(function(winTable)
		if (bedwars.Store:getState().Game.myTeam or {}).id == winTable.winningTeamId or lplr.Neutral then
			wins:Increment()
		end
	end))

	fable:Clean(fableEvents.EntityDeathEvent.Event:Connect(function(deathTable)
	...
	pcall(function()
		if getthreadidentity and setthreadidentity then
			...
			task.spawn(function()
				repeat
					task.wait(0.1)
				until fable.Loaded == nil or bedwars.AppController:isAppOpen('BedwarsItemShopApp')
...
run(function()
	...
	AutoClicker = fable.Categories.Combat:CreateModule({
		Name = 'AutoClicker',
		Function = function(callback)
			if callback then
...
run(function()
	...
	fable.Categories.Combat:CreateModule({
		Name = 'NoClickDelay',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Reach = fable.Categories.Combat:CreateModule({
		Name = 'Reach',
		Function = function(callback)
...
run(function()
	...
	Sprint = fable.Categories.Combat:CreateModule({
		Name = 'Sprint',
		Function = function(callback)
			if callback then
				if inputService.TouchEnabled then 
...
run(function()
	...
	TriggerBot = fable.Categories.Combat:CreateModule({
		Name = 'TriggerBot',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Velocity = fable.Categories.Combat:CreateModule({
		Name = 'Velocity',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AntiFall = fable.Categories.Blatant:CreateModule({
		Name = 'AntiFall',
		Function = function(callback)
			if callback then
							...
							if Mode.Value == 'Normal' then
								...
								if top then
									...
									connection = runService.PreSimulation:Connect(function()
										if fable.Modules.Fly.Enabled or fable.Modules.InfiniteFly.Enabled or fable.Modules.LongJump.Enabled then
...
run(function()
	...
	FastBreak = fable.Categories.Blatant:CreateModule({
		Name = 'FastBreak',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Fly = fable.Categories.Blatant:CreateModule({
		Name = 'Fly',
		Function = function(callback)
			...
			if callback then
				...
				Fly:Clean(fableEvents.AttributeChanged.Event:Connect(function(changed)
...
run(function()
	...
	HitBoxes = fable.Categories.Blatant:CreateModule({
		Name = 'HitBoxes',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local anims, AnimDelay, AnimTween, armC0 = fable.Libraries.auraanims, tick()
	...
	Killaura = fable.Categories.Blatant:CreateModule({
		Name = 'Killaura',
		Function = function(callback)
			if callback then
				if inputService.TouchEnabled then
...
run(function()
	...
	local LongJumpMethods = {
		...
		cat = function(_, _, dir)
			LongJump:Clean(fableEvents.CatPounce.Event:Connect(function()
	...
	LongJump = fable.Categories.Blatant:CreateModule({
		Name = 'LongJump',
		Function = function(callback)
			frictionTable.LongJump = callback or nil
			updateVelocity()
			if callback then
				LongJump:Clean(fableEvents.EntityDamageEvent.Event:Connect(function(damageTable)
				...
				LongJump:Clean(fableEvents.GrapplingHookFunctions.Event:Connect(function(dataTable)
...
run(function()
	...
	NoFall = fable.Categories.Blatant:CreateModule({
		Name = 'NoFall',
		Function = function(callback)
			if callback then
				local tracked = 0
...
run(function()
	...
	local ProjectileAimbot = fable.Categories.Blatant:CreateModule({
...
run(function()
	...
	ProjectileAura = fable.Categories.Blatant:CreateModule({
		Name = 'ProjectileAura',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Speed = fable.Categories.Blatant:CreateModule({
		Name = 'Speed',
		Function = function(callback)
...
run(function()
	...
	BedESP = fable.Categories.Render:CreateModule({
		Name = 'BedESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Health = fable.Categories.Render:CreateModule({
		Name = 'Health',
		Function = function(callback)
			if callback then
				...
				label.Parent = fable.gui
				Health:Clean(label)
				Health:Clean(fableEvents.AttributeChanged.Event:Connect(function()
					label.Text = entitylib.isAlive and math.round(lplr.Character:GetAttribute('Health'))..' ❤️' or ''
					label.TextColor3 = entitylib.isAlive and Color3.fromHSV((lplr.Character:GetAttribute('Health') / lplr.Character:GetAttribute('MaxHealth')) / 2.8, 0.86, 1) or Color3.new()
				end))
			end
...
run(function()
	...
	KitESP = fable.Categories.Render:CreateModule({
		Name = 'KitESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	NameTags = fable.Categories.Render:CreateModule({
		Name = 'NameTags',
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					NameTags:Clean(fable.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	StorageESP = fable.Categories.Render:CreateModule({
		Name = 'StorageESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBalloon = fable.Categories.Utility:CreateModule({
		Name = 'AutoBalloon',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoKit = fable.Categories.Utility:CreateModule({
		Name = 'AutoKit',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoPearl = fable.Categories.Utility:CreateModule({
		Name = 'AutoPearl',
		Function = function(callback)
			if callback then
				local check
...
run(function()
	...
	AutoPlay = fable.Categories.Utility:CreateModule({
		Name = 'AutoPlay',
		Function = function(callback)
			if callback then
				AutoPlay:Clean(fableEvents.EntityDeathEvent.Event:Connect(function(deathTable)
					if deathTable.finalKill and deathTable.entityInstance == lplr.Character and isEveryoneDead() and store.matchState ~= 2 then
						joinQueue()
					end
				end))
				AutoPlay:Clean(fableEvents.MatchEndEvent.Event:Connect(joinQueue))
			end
		end,
...
run(function()
	...
	AutoToxic = fable.Categories.Utility:CreateModule({
		Name = 'AutoToxic',
		Function = function(callback)
			if callback then
				AutoToxic:Clean(fableEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
				...
				AutoToxic:Clean(fableEvents.EntityDeathEvent.Event:Connect(function(deathTable)
				...
				AutoToxic:Clean(fableEvents.MatchEndEvent.Event:Connect(function(winstuff)
					if GG.Enabled then
						if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
...
run(function()
	...
	AutoVoidDrop = fable.Categories.Utility:CreateModule({
		Name = 'AutoVoidDrop',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MissileTP = fable.Categories.Utility:CreateModule({
		Name = 'MissileTP',
		Function = function(callback)
			if callback then
				MissileTP:Toggle()
...
run(function()
	...
	PickupRange = fable.Categories.Utility:CreateModule({
		Name = 'PickupRange',
		Function = function(callback)
			if callback then
...
run(function()
	...
	RavenTP = fable.Categories.Utility:CreateModule({
		Name = 'RavenTP',
		Function = function(callback)
			if callback then
				RavenTP:Toggle()
...
run(function()
	...
	Scaffold = fable.Categories.Utility:CreateModule({
		Name = 'Scaffold',
		Function = function(callback)
			if label then
...
run(function()
	...
	ShopTierBypass = fable.Categories.Utility:CreateModule({
		Name = 'ShopTierBypass',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local function staffFunction(plr, checktype)
		if not fable.Loaded then
			repeat task.wait() until fable.Loaded
		end
		...
		elseif Mode.Value == 'Profile' then
			...
			if fable.Profile ~= Profile.Value then
				fable:Load(true, Profile.Value)
			end
	...
	local function playerAdded(plr)
		...
		elseif getRole(plr, 5774246) >= 100 then
			...
			if table.find(blacklistedclans, plr:GetAttribute('ClanTag')) and fable.Loaded and Clans.Enabled then
				connection:Disconnect()
	...
	StaffDetector = fable.Categories.Utility:CreateModule({
		Name = 'StaffDetector',
		Function = function(callback)
			if callback then
	...
	task.spawn(function()
		repeat task.wait(1) until fable.Loaded or fable.Loaded == nil
		if fable.Loaded and not StaffDetector.Enabled then
...
run(function()
	fable.Categories.World:CreateModule({
		Name = 'Anti-AFK',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoSuffocate = fable.Categories.World:CreateModule({
		Name = 'AutoSuffocate',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AutoTool = fable.Categories.World:CreateModule({
		Name = 'AutoTool',
		Function = function(callback)
			if callback then
...
run(function()
	...
	BedPlates = fable.Categories.World:CreateModule({
		Name = 'BedPlates',
		Function = function(callback)
			if callback then
				...
				BedPlates:Clean(fableEvents.PlaceBlockEvent.Event:Connect(refreshNear))
				BedPlates:Clean(fableEvents.BreakBlockEvent.Event:Connect(refreshNear))
...
run(function()
	...
	BedProtector = fable.Categories.World:CreateModule({
		Name = 'BedProtector',
		Function = function(callback)
			if callback then
...
run(function()
			...
			local mounted = bedwars.Roact.mount(create('BillboardGui', {
				...
				create('Frame', {
					...
					create('ImageLabel', {
						...
						Image = getcustomasset('FableV4/assets/new/blur.png'),
	...
	Breaker = fable.Categories.World:CreateModule({
		Name = 'Breaker',
		Function = function(callback)
			if callback then
				for _ = 1, 30 do
...
run(function()
	...
	ChestSteal = fable.Categories.World:CreateModule({
		Name = 'ChestSteal',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Schematica = fable.Categories.World:CreateModule({
		Name = 'Schematica',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ArmorSwitch = fable.Categories.Inventory:CreateModule({
		Name = 'ArmorSwitch',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBank = fable.Categories.Inventory:CreateModule({
		Name = 'AutoBank',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBuy = fable.Categories.Inventory:CreateModule({
		Name = 'AutoBuy',
		Function = function(callback)
			if callback then
				repeat task.wait() until store.queueType ~= 'bedwars_test'
				...
				local lastupgrades
				AutoBuy:Clean(fableEvents.InventoryAmountChanged.Event:Connect(function()
	...
	AutoConsume = fable.Categories.Inventory:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				AutoConsume:Clean(fableEvents.AttributeChanged.Event:Connect(function(attribute)
...
run(function()
	...
	local function CreateWindow(self)
		...
		window.Parent = fable.gui.ScaledGui
		...
		close.Image = getcustomasset('FableV4/assets/new/close.png')
		...
		close.MouseButton1Click:Connect(function()
			window.Visible = false
			fable.gui.ScaledGui.ClickGui.Visible = true
		end)
		...
		searchicon.Image = getcustomasset('FableV4/assets/new/search.png')
		...
		windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
			...
			children.CanvasSize = UDim2.fromOffset(0, windowlist.AbsoluteContentSize.Y / fable.guiscale.Scale)
		end)
	...
	fable.Components.HotbarList = function(optionsettings, children, api)
		if fable.ThreadFix then
			setthreadidentity(8)
		end
		local optionapi = {
		...
		textbuttonicon.Image = getcustomasset('FableV4/assets/new/add.png')
		...
		windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
			...
			hotbarlist.Size = UDim2.fromOffset(220, math.min(43 + windowlist.AbsoluteContentSize.Y / fable.guiscale.Scale, 603))
		end)
		...
		function optionapi:AddHotbar(data)
			...
			hotbar.MouseButton1Click:Connect(function()
				...
				if ind == optionapi.Selected then
					fable.gui.ScaledGui.ClickGui.Visible = false
			...
			close.BackgroundColor3 = Color3.new(1, 1, 1)
			close.BackgroundTransparency = 1
			close.Image = getcustomasset('FableV4/assets/new/closemini.png')
			close.ImageColor3 = color.Light(uipallet.Text, 0.2)
			close.ImageTransparency = 0.5
			close.AutoButtonColor = false
			close.Parent = hotbar
	...
	AutoHotbar = fable.Categories.Inventory:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				AutoHotbar:Clean(fableEvents.InventoryAmountChanged.Event:Connect(sortCallback))
			end
		end,
...
run(function()
	...
	local FastConsume = fable.Categories.Inventory:CreateModule({
		Name = 'FastConsume',
		Function = function(callback)
			if callback then
...
run(function()
	...
	FastDrop = fable.Categories.Inventory:CreateModule({
		Name = 'FastDrop',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	BedBreakEffect = fable.Legit:CreateModule({
		Name = 'Bed Break Effect',
		Function = function(callback)
			if callback then
	            BedBreakEffect:Clean(fableEvents.BedwarsBedBreak.Event:Connect(function(data)
...
run(function()
	...
	local Crosshair = fable.Legit:CreateModule({
		Name = 'Crosshair',
		Function = function(callback)
			if callback then
...
run(function()
	...
	DamageIndicator = fable.Legit:CreateModule({
...
run(function()
	...
	FPSBoost = fable.Legit:CreateModule({
		Name = 'FPS Boost',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HitColor = fable.Legit:CreateModule({
		Name = 'Hit Color',
		Function = function(callback)
			if callback then 
				repeat
...
run(function()
	...
	Interface = fable.Legit:CreateModule({
		Name = 'Interface',
		Function = function(callback)
...
run(function()
	...
	KillEffect = fable.Legit:CreateModule({
		Name = 'Kill Effect',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ReachDisplay = fable.Legit:CreateModule({
		Name = 'Reach Display',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SongBeats = fable.Legit:CreateModule({
		Name = 'Song Beats',
		Function = function(callback)
			if callback then
...
run(function()
	...
	SoundChanger = fable.Legit:CreateModule({
		Name = 'SoundChanger',
		Function = function(callback)
			if callback then
...
run(function()
	...
	UICleanup = fable.Legit:CreateModule({
		Name = 'UI Cleanup',
		Function = function(callback)
			for i, v in (callback and new or old) do
				for i2, v2 in v do
...
run(function()
	...
	Viewmodel = fable.Legit:CreateModule({
		Name = 'Viewmodel',
		Function = function(callback)
...
run(function()
	...
	WinEffect = fable.Legit:CreateModule({
		Name = 'WinEffect',
		Function = function(callback)
			if callback then
				WinEffect:Clean(fableEvents.MatchEndEvent.Event:Connect(function()
