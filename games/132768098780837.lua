...
run(function()
	...
	blocks = collection('BedWarsX_PlacedBlock', fable, function(tab, block)
	...
	fable:Clean(lplr:GetAttributeChangedSignal('RoundKills'):Connect(function()
		if lplr:GetAttribute('RoundKills') > 0 then
			kills:Increment()
		end
	end))

	fable:Clean(bw.RemoteIndex.Round_Event.OnClientEvent:Connect(function(data)
	...
	fable:Clean(bw.RemoteIndex.Bed_Destroyed.OnClientEvent:Connect(function(data)
		if type(data) == 'table' and data.breakerId == lplr.UserId then
			beds:Increment()
		end
	end))

	fable:Clean(entitylib.Events.EntityAdded:Connect(function(entity)
	...
	fable:Clean(entitylib.Events.EntityRemoving:Connect(function(entity)
...
run(function()
	...
	AnticheatBypass = fable.Categories.Blatant:CreateModule({
...
run(function()
	...
	AutoLeave = fable.Categories.Utility:CreateModule({
		Name = 'AutoLeave',
		Function = function(callback)
			if callback then
...
run(function()
	...
	FastBreak = fable.Categories.World:CreateModule({
		Name = 'FastBreak',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HideShield = fable.Legit:CreateModule({
		Name = 'HideShield',
		Function = function(callback)
			if callback then
