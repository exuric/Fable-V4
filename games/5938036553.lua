...
if not select(1, ...) and game.PlaceId == 5938036553 then
	if run_on_actor and getactors then
		...
		task.spawn(function()
			...
			notif('Fable', 'Failed to find actor', 10, 'alert')
		end)
	else
		fable.Load = function()
...
local function hookEvent(id, rfunc)
	...
	if not suc then
		notif('Fable', 'Failed to hook ('..id..')', 10, 'alert')
	end
...
run(function()
	...
	fable:Clean(Drawing.kill or function() end)
...
run(function()
	...
	entitylib.getEntityColor = function(ent)
		if not (ent.Player and fable.Settings.Modules.Options['Use team color'].Enabled) then return end
...
run(function()
	...
	GrenadeTP = fable.Categories.Blatant:CreateModule({
		Name = 'GrenadeTP',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	GrenadeESP = fable.Categories.Render:CreateModule({
		Name = 'GrenadeESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	NoHurtCam = fable.Categories.Render:CreateModule({
		Name = 'NoHurtCam',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ThirdPerson = fable.Categories.Render:CreateModule({
		Name = 'ThirdPerson',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoRespawn = fable.Categories.Utility:CreateModule({
		Name = 'AutoRespawn',
		Function = function(callback)
			if callback then
