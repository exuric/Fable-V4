...
local drawingactor = loadstring(downloadFile('FableV4/libraries/drawing.lua'), 'drawing')(...)
local redline = {Teams = {}}
...
local function warningRoutine(hash)
	local path = 'FableV4/profiles/agreementhash.txt'
...
if not select(1, ...) then
	if run_on_actor then
		local oldreload = shared.fablereload
		fable.Load = function()
		...
		task.spawn(function()
			repeat task.wait() until not shared.fable
			local executionString = "loadfile('FableV4/main.lua')("..drawingactor..")"
			...
			if oldreload then
				executionString = 'shared.fablereload = true\n'..executionString
			end
		...
		fable.Load = function()
			notif('Fable', 'Missing actor functions.', 10, 'alert')
		end
	end
	...
	if game.PlaceId == 126691165749976 then
		...
		local function processPlayer(plr)
			if tonumber(plr.Name) then
				...
				fable:Clean(plr:GetAttributeChangedSignal('team_id'):Connect(function()
					redline.Teams[plr.Name] = plr:GetAttribute('team_id')
					task.spawn(updatePlayer, plr)
				end))
			end
		...
		local function processMatch(match)
			if match and match.Name == 'Match' then
				fable:Clean(match.DescendantAdded:Connect(processPlayer))
		...
		fable:Clean(replicatedStorage.ReadOnly.ChildAdded:Connect(processMatch))
...
run(function()
	...
	for _, v in getloadedmodules() do
		if v:GetFullName() == 'Start.Client.ClientRoot' then
			...
			if not rawget(root, 'loaded') then
				repeat
					task.wait()
				until rawget(root, 'loaded') or fable.Loaded == nil
			end
	...
	if redline.ActionEventPacket then
		fable:Clean(redline.ActionEventPacket.OnClientEvent:Connect(function(data)
	...
	fable:Clean(fableEvents.MatchEnded.Event:Connect(function(won)
		if won then
			wins:Increment()
		end
	end))

	fable:Clean(lplr.PlayerGui.ChildAdded:Connect(function(obj)
		if obj.Name == 'MatchResultsScreen' then
			...
			if obj then
				...
				fableEvents.MatchEnded:Fire(obj.Text:find('WON') and true or false, results)
			end
		end
	end))
end)
...
run(function()
	...
	AntiParry = fable.Categories.Blatant:CreateModule({
		Name = 'AntiParry',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoParry = fable.Categories.Blatant:CreateModule({
		Name = 'AutoParry',
		Function = function(callback)
			if callback then
...
run(function()
	...
	InfiniteDash = fable.Categories.Blatant:CreateModule({
		Name = 'InfiniteDash',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoToxic = fable.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			if callback then
				AutoToxic:Clean(fableEvents.MatchEnded.Event:Connect(function(won)
					if GG.Enabled then
						if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
							if textChatService:CanUserChatAsync(lplr.UserId) then
...
run(function()
	...
	HitSound = fable.Legit:CreateModule({
		...
		Function = function(callback)
			if callback then
				HitSound:Clean(fableEvents.Hit.Event:Connect(function()
					if #sounds > 0 then
...
run(function()
	...
	KillSound = fable.Legit:CreateModule({
		...
		Function = function(callback)
			if callback then
				KillSound:Clean(fableEvents.PlayerKill.Event:Connect(function()
					if #sounds > 0 then
						local obj = Instance.new('Sound')
						obj.SoundId = sounds[math.random(1, #sounds)]
						obj.PlayOnRemove = true
