...
run(function()
	...
	if not lstats then
		repeat
			lstats = lplr:FindFirstChild('TempPlayerStatsModule')
			task.wait()
		until lstats or fable.Loaded == nil
	...
	local function updateMap()
		if mapval.Value then
			mapobj = mapval.Value
			fableEvents.MapAdded:Fire(mapobj)
		elseif mapboj then
			fableEvents.MapRemoved:Fire(mapboj)
			mapobj = nil
		end
	end

	fable:Clean(mapval:GetPropertyChangedSignal('Value'):Connect(updateMap))
	if mapval.Value then
		updateMap()
	end
end)
...
run(function()
	...
	PhaseHammer = fable.Categories.Blatant:CreateModule({
		Name = 'PhaseHammer',
		Function = function(callback)
			if callback then
...
run(function()
	...
	RestrainBeast = fable.Categories.Blatant:CreateModule({
		Name = 'RestrainBeast',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SlowBeast = fable.Categories.Blatant:CreateModule({
		Name = 'SlowBeast',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SpamBeast = fable.Categories.Blatant:CreateModule({
		Name = 'SpamBeast',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	ComputerESP = fable.Categories.Render:CreateModule({
		Name = 'ComputerESP',
		Function = function(callback)
			if callback then
				ComputerESP:Clean(fableEvents.MapAdded.Event:Connect(MapAdded))
				ComputerESP:Clean(fableEvents.MapRemoved.Event:Connect(function()
...
run(function()
	...
	AutoComputer = fable.Categories.Utility:CreateModule({
		Name = 'AutoComputer',
		Function = function(callback)
			if callback then
