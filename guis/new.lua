...
do
	...
	function fable:TextColor(h, s, v)
...
do
	local data = isfile('FableV4/profiles/color.txt') and loadJson('FableV4/profiles/color.txt')
	if data then
...
local function addBlur(parent, notif, old)
	...
	if old then
		...
		blur.Image = getfableasset('FableV4/assets/new/'..(notif and 'blurnoti' or 'blur')..'.png')
...
local function addCloseButton(parent, mini, offset)
	...
	close.Image = getfableasset('FableV4/assets/new/'..(mini and 'closemini' or 'close')..'.png')
...
function fable:CreateCategory(props)
	return components.Category(props)
end

function fable:CreateCategoryList(props)
	return components.CategoryList(props)
end

function fable:CreateNotification(title, text, duration, type)
	...
	task.delay(0, function()
		...
		notification.Image = getfableasset('FableV4/assets/new/notification.png')
		...
		iconshadow.Image = getfableasset('FableV4/assets/new/noti_'..(type or 'info')..'.png')
...
function fable:CreateOverlay(props)
	return components.Overlay(props)
end

function fable:Load(skipgui, profile)
	local guiData = {Categories = {}}
	local oldProfile = self.Profile
	local canSave = true
	local toggleCount = 0

	if isfile('FableV4/profiles/'..game.GameId..'.gui.txt') then
		guiData = loadJson('FableV4/profiles/'..game.GameId..'.gui.txt')
		if not guiData then
			guiData = {Categories = {}}
			self:CreateNotification('Fable', 'Failed to load GUI settings.', 10, 'alert')
			canSave = false
		end
	...
	if isfile('FableV4/profiles/'..self.Profile..self.Place..'.txt') then
		local mainData = loadJson('FableV4/profiles/'..self.Profile..self.Place..'.txt')
		if not mainData then
			mainData = {Categories = {}, Modules = {}, Legit = {}}
			self:CreateNotification('Fable', 'Failed to load '..self.Profile..' profile.', 10, 'alert')
			canSave = false
		end
	...
	if inputService.TouchEnabled and not skipgui then
		...
		image.Image = getfableasset('FableV4/assets/new/fable.png')
...
function fable:LoadOptions(obj, data)
...
function fable:LoadGUI()
	...
	run(function()
		...
		TextGUI = fable:CreateOverlay({
			Name = 'Text GUI',
			Icon = getfableasset('FableV4/assets/new/textgui.png'),
		...
		ColorSlider = TextGUI:CreateColorSlider({
			Name = 'Text GUI color',
			Function = function()
				fable:UpdateGUI(fable.GUIColor.Hue, fable.GUIColor.Sat, fable.GUIColor.Value)
			end,
			Darker = true,
			Visible = false
		})
		...
		Watermark = TextGUI:CreateToggle({
			Name = 'Watermark',
			Tooltip = 'Renders a fable watermark',
			Function = function()
				fable:UpdateTextGUI()
			end
		})
		...
		Logo.Image = getfableasset('FableV4/assets/new/fable.png')
		Logo.Name = 'Logo'
		...
		LogoV4.Image = getfableasset('FableV4/assets/new/v4.png')
		LogoV4.Name = 'Logo2'
		...
		fable:Clean(TextGUI.Children:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
			if fable.ThreadFix then
				setthreadidentity(8)
			end
		...
		function fable:UpdateTextGUI(afterload)
			if not afterload and not fable.Loaded then return end
			if TextGUI.Button.Enabled then
				...
				for name, module in fable.Modules do
		...
		function TextGUI:UpdateColor(hue, sat, val, default)
			LogoGradient.Color = ColorSequence.new({
				...
				ColorSequenceKeypoint.new(1, Gradient.Enabled and Color3.fromHSV(fable:Color((hue - 0.075) % 1)) or Color3.fromHSV(hue, sat, val))
			})
			...
			for index, label in Labels do
				label.Text.TextColor3 = isCustom or (fable.GUIColor.Rainbow and Color3.fromHSV(fable:Color((hue - ((Gradient.Enabled and index + 2 or index) * 0.025)) % 1)) or LogoGradient.Color.Keypoints[2].Value)
	...
	run(function()
		...
		TargetInfoOverlay = fable:CreateOverlay({
			Name = 'Target Info',
			Icon = getfableasset('FableV4/assets/new/targetinfo.png'),
			Size = UDim2.fromOffset(14, 14),
			Position = UDim2.fromOffset(12, 14),
			CategorySize = 240,
			Function = function(callback)
		...
		function targetinfo:Update()
			local entitylib = fable.Libraries
		...
		fable.Libraries.targetinfo = targetinfo
	end)
...
function fable:Save(newProfile)
	...
	writefile('FableV4/profiles/'..game.GameId..'.gui.txt', httpService:JSONEncode(guiData))
	writefile('FableV4/profiles/'..self.Profile..self.Place..'.txt', httpService:JSONEncode(mainData))
end

function fable:SaveOptions(obj)
	local data = {}
...
function fable:Uninject()
	...
	shared.fable = nil
	shared.fablereload = nil
	shared.FableIndependent = nil
end

function fable:UpdateGUI(hue, sat, val, default)
	if fable.Loaded == nil then return end
	if not default and fable.GUIColor.Rainbow then return end

	if TextGUI.Button.Enabled then
		TextGUI:UpdateColor(hue, sat, val, default)
	end

	if not clickgui.Visible and not fable.Legit.Window.Visible then return end
	local isRainbow = fable.GUIColor.Rainbow and fable.RainbowMode.Value ~= 'Retro'

	for name, component in fable.Categories do
		component:Color(hue, sat, val, isRainbow)
	end

	for _, component in fable.Modules do
		component:Color(hue, sat, val, isRainbow)
	end

	for _, component in fable.Overlays.Options do
		if component.Color then
			component:Color(hue, sat, val, isRainbow)
		end
	end

	for _, pane in fable.Settings do
	...
	if fable.Legit.Window.Visible then
		for _, component in fable.Legit.Modules do
			component:Color(hue, sat, val, isRainbow)
		end
	end
end
...
components = {
	Bind = function(props, children, api)
		...
		icon.Image = getfableasset('FableV4/assets/new/bind.png')
		...
		if props.Module then
			if props.Cover then
				...
				cover.Image = getfableasset('FableV4/assets/new/bindbkg.png')
				cover.Name = 'Cover'
		...
		function component:Destroy()
			...
			local index = table.find(fable.ActiveBinds, self)
			if index then
				table.remove(fable.ActiveBinds, index)
			end
		end
		...
		function component:SetBind(keys, mouse)
			...
			if mouse then
				icon.Image = getfableasset('FableV4/assets/new/edit.png')
			...
			if #keys <= 0 then
				...
				local index = table.find(fable.ActiveBinds, component)
				if index then
					table.remove(fable.ActiveBinds, index)
				end
			else
				...
				if not table.find(fable.ActiveBinds, component) then
					table.insert(fable.ActiveBinds, component)
				end
			end
		end
		...
		bind.MouseEnter:Connect(function()
			...
			icon.Image = getfableasset(component.Binding and 'FableV4/assets/new/close.png' or 'FableV4/assets/new/edit.png')
		...
		bind.MouseLeave:Connect(function()
			...
			icon.Visible = not label.Visible
			icon.Image = getfableasset(component.Binding and 'FableV4/assets/new/close.png' or 'FableV4/assets/new/bind.png')
		...
		bind.MouseButton1Click:Connect(function()
			if fable.Binding then
				if fable.Binding == component then
			...
			icon.Image = getfableasset('FableV4/assets/new/close.png')
			fable.Binding = component
		end)
	...
	Category = function(props, children, api)
		...
		pencil.Image = getfableasset('FableV4/assets/new/editlarge.png')
		...
		arrow.Image = getfableasset('FableV4/assets/new/downexpand.png')
		...
		window.MouseEnter:Connect(function()
			pencilbutton.Visible = not fable.EditGUI
		end)
		...
		component.Button = fable.Categories.Main:CreateGUIButton({
			Name = props.Name,
			Icon = props.Icon,
			Size = props.Size,
			Window = window
		...
		fable.Categories[props.Name] = component
	...
	CategoryList = function(props, children, api)
		...
		settings.Image = getfableasset('FableV4/assets/new/settings.png')
		...
		addbutton.Image = getfableasset('FableV4/assets/new/add.png')
		...
		function component:CreateProfile(value, data)
			...
			profile.Bind.Triggered:Connect(function(isPressed)
				if isPressed and fable.Profile ~= value then
					fable:Save(value)
					fable:Load(true)
					self:ChangeValue()
				end
		...
		function component:ChangeValue(value, skipGUI)
				...
				if props.Profiles then
					...
					if index then
						if value ~= 'default' then
							...
							if isfile('FableV4/profiles/'..value..fable.Place..'.txt') and delfile then
								delfile('FableV4/profiles/'..value..fable.Place..'.txt')
							end
						end
					else
			...
			for _, name in self.List do
				if props.Profiles then
					...
					dots.Image = getfableasset('FableV4/assets/new/settingdots.png')
					...
					name.Enabled = name.Name == fable.Profile
					...
					close.BackgroundColor3 = Color3.new(1, 1, 1)
					close.BackgroundTransparency = 1
					close.Image = getfableasset('FableV4/assets/new/closetiny.png')
					close.ImageColor3 = color.Light(uipallet.Text, 0.2)
					close.ImageTransparency = 0.5
		...
		function component:Color(hue, sat, val, isRainbow)
			...
			addbutton.ImageColor3 = isRainbow and Color3.fromHSV(fable:Color(hue % 1)) or Color3.fromHSV(hue, sat, val)
		
			if self.Selected then
				self.Selected.BackgroundColor3 = isRainbow and Color3.fromHSV(fable:Color(hue % 1)) or Color3.fromHSV(hue, sat, val)
				self.Selected.Title.TextColor3 = fable.GUIColor.Rainbow and Color3.new(0.19, 0.19, 0.19) or fable:TextColor(hue, sat, val)
		...
		function component:Load(data)
			fable:LoadOptions(self, data.Options)
		...
		function component:Save(data)
			data[props.Name] = {
				...
				Options = fable:SaveOptions(self),
				Position = {
					X = window.Position.X.Offset,
					Y = window.Position.Y.Offset
				}
	...
	ColorSlider = function(props, children, api)
		...
		preview.Image = getfableasset('FableV4/assets/new/colorpreview.png')
		...
		local icon = Instance.new('ImageLabel')
		icon.BackgroundTransparency = 1
		icon.Image = getfableasset('FableV4/assets/new/downexpandslider.png')
		icon.ImageColor3 = color.Dark(uipallet.Text, 0.43)
		...
		ring1.Image = getfableasset('FableV4/assets/new/rainbow_1.png')
		...
		ring2.Image = getfableasset('FableV4/assets/new/rainbow_2.png')
		ring2.Parent = rainbow
		local ring3 = Instance.fromExisting(ring1)
		ring3.Image = getfableasset('FableV4/assets/new/rainbow_3.png')
		ring3.Parent = rainbow
		local ring4 = Instance.fromExisting(ring1)
		ring4.Image = getfableasset('FableV4/assets/new/rainbow_4.png')
		...
		function component:Toggle()
			...
			if self.Rainbow then
				table.insert(fable.RainbowSliders, self)
				...
				local index = table.find(fable.RainbowSliders, self)
				if index then
					table.remove(fable.RainbowSliders, index)
				end
	...
	Dropdown = function(props, children, api)
		...
		local arrow = Instance.new('ImageLabel')
		arrow.BackgroundTransparency = 1
		arrow.Image = getfableasset('FableV4/assets/new/expandarrow.png')
		arrow.ImageColor3 = Color3.fromRGB(140, 140, 140)
	...
	GUI = function(props, children, api)
		...
		v4logo.Image = getfableasset('FableV4/assets/new/v4mini.png')
		v4logo.Name = 'V4Logo'
		...
		settingsicon.Image = getfableasset('FableV4/assets/new/settings.png')
		...
		discord.Image = getfableasset('FableV4/assets/new/discord.png')
		...
		function component:Color(hue, sat, val, isRainbow)
			...
			for _, button in self.Buttons do
				if button.Enabled then
					button.Object.TextColor3 = isRainbow and Color3.fromHSV(fable:Color((hue - (button.Index * 0.025)) % 1)) or Color3.fromHSV(hue, sat, val)
		...
		function component:Load(data)
			for name, paneData in data.Settings do
				local pane = fable.Settings[name]
				if pane then
					pane:Load(paneData)
				end
			end
		...
		function component:Save(data)
			...
			for name, pane in fable.Settings do
		...
		fable.Categories.Main = component
	...
	GUIButton = function(props, children, api)
		...
		function component:Toggle()
			if props.Window then
				...
				button.TextColor3 = self.Enabled and Color3.fromHSV(fable.GUIColor.Hue, fable.GUIColor.Sat, fable.GUIColor.Value) or uipallet.Text
				if icon then
	...
	GUISlider = function(props, children, api)
		...
		knob.Image = getfableasset('FableV4/assets/new/theme.png')
		...
		local normalknob = getfableasset('FableV4/assets/new/theme.png')
		local rainbowknob = getfableasset('FableV4/assets/new/customtheme.png')
		local rainbowthread
		local currentNotch
	...
	ImageToggle = function(props, children, api)
		...
		function component:Color(hue, sat, val, isRainbow)
			if self.Enabled then
				tween:Cancel(holder)
				holder.BackgroundColor3 = isRainbow and Color3.fromHSV(fable:Color((hue - (self.Index * 0.075)) % 1)) or Color3.fromHSV(hue, sat, val)
			end
		end
		...
		function component:Toggle()
			...
			tween:Tween(holder, uipallet.Tween, {
				BackgroundColor3 = self.Enabled and (isRainbow and Color3.fromHSV(fable:Color((fable.GUIColor.Hue - (self.Index * 0.075)) % 1)) or Color3.fromHSV(fable.GUIColor.Hue, fable.GUIColor.Sat, fable.GUIColor.Value)) or (isHover and color.Light(uipallet.Main, 0.37) or color.Light(uipallet.Main, 0.14))
			})
	...
	LegitModule = function(props, children, api)
		...
		local dots = Instance.new('ImageLabel')
		dots.BackgroundTransparency = 1
		dots.Image = getfableasset('FableV4/assets/new/overlaydots.png')
		dots.ImageColor3 = color.Light(uipallet.Main, 0.37)
		dots.Name = 'Dots'
		...
		back.Image = getfableasset('FableV4/assets/new/back.png')
		...
		function component:Toggle()
			...
			tween:Tween(holder, uipallet.Tween, {
				BackgroundColor3 = self.Enabled and Color3.fromHSV(fable.GUIColor.Hue, fable.GUIColor.Sat, fable.GUIColor.Value) or color.Light(uipallet.Main, 0.14)
			})
		...
		shadow:GetPropertyChangedSignal('Visible'):Connect(function()
			tooltip.Visible = false
			fable.LegitVisible = shadow.Visible
		end)
	...
	LegitWindow = function(props, children, api)
		...
		table.insert(fable.Windows, window)
		...
		local icon = Instance.new('ImageLabel')
		icon.BackgroundTransparency = 1
		icon.Image = getfableasset('FableV4/assets/new/legit_mode_icon.png')
		icon.ImageColor3 = uipallet.Text
		...
		local close = Instance.new('ImageButton')
		close.BackgroundTransparency = 1
		close.Image = getfableasset('FableV4/assets/new/min.png')
		...
		searchicon.Image = getfableasset('FableV4/assets/new/search.png')
		...
		fable:Clean(clickgui:GetPropertyChangedSignal('Visible'):Connect(visibleCheck))
	...
	Module = function(props, children, api)
		...
		local component = {
			...
			Index = getTableSize(fable.Modules),
			Name = props.Name,
			Options = {},
			Visible = true
		}
		...
		function component:Color(hue, sat, val, isRainbow)
			if self.Enabled then
				button.BackgroundColor3 = isRainbow and Color3.fromHSV(fable:Color((hue - (self.Index * 0.025)) % 1)) or Color3.fromHSV(hue, sat, val)
				button.TextColor3 = fable.GUIColor.Rainbow and Color3.new(0.19, 0.19, 0.19) or fable:TextColor(hue, sat, val)
				button.UIGradient.Enabled = isRainbow and fable.RainbowMode.Value == 'Gradient'
		
				if button.UIGradient.Enabled then
					button.BackgroundColor3 = Color3.new(1, 1, 1)
					button.UIGradient.Color = ColorSequence.new({
						ColorSequenceKeypoint.new(0, Color3.fromHSV(fable:Color((hue - (self.Index * 0.025)) % 1))),
						ColorSequenceKeypoint.new(1, Color3.fromHSV(fable:Color((hue - ((self.Index + 1) * 0.025)) % 1)))
					})
				end
			...
			if self.Visible then
				editbox.BackgroundColor3 = isRainbow and Color3.fromHSV(fable:Color((hue - (self.Index * 0.025)) % 1)) or Color3.fromHSV(hue, sat, val)
		...
		function component:SetVisible(isVisible, isLoad)
			...
			if isLoad and not fable.EditGUI then
		...
		function component:Toggle(multiple)
			...
			if multiple then
				if not fable.TextGUIThread then
					fable.TextGUIThread = task.defer(function()
						if fable.Loaded ~= nil then
							fable:UpdateTextGUI()
						end
		
						fable.TextGUIThread = nil
					end)
				end
			else
				fable:UpdateTextGUI()
		...
		bind.Triggered:Connect(function(isDown)
			if bind.Hold then
				if component.Enabled ~= isDown then
					if fable.ToggleNotifications.Enabled then
						fable:CreateNotification(props.Name, (not component.Enabled and "<font color='#00AA00'>Enabled</font>" or "<font color='#FF5A5A'>Disabled</font>"), 1.5)
					end
		...
		fable.Modules[props.Name] = component
		fable:SortCategories()
	...
	Overlay = function(props, children, api)
		...
		local component
		component = {
			Button = fable.Overlays:CreateImageToggle({
				Name = props.Name,
				Function = function(callback)
		...
		pin.Image = getfableasset('FableV4/assets/new/pin.png')
	...
	OverlayBar = function(props, children, api)
		...
		button.Image = getfableasset('FableV4/assets/new/overlays.png')
		...
		local icon = Instance.new('ImageLabel')
		icon.BackgroundTransparency = 1
		icon.Image = getfableasset('FableV4/assets/new/overlayslarge.png')
		icon.ImageColor3 = uipallet.Text
	...
	SearchBar = function(props, children, api)
		...
		local icon = Instance.new('ImageLabel')
		icon.BackgroundTransparency = 1
		icon.Image = getfableasset('FableV4/assets/new/search.png')
		...
		legiticon.Image = getfableasset('FableV4/assets/new/legit_switch.png')
		legiticon.Name = 'Legit'
		legiticon.Position = UDim2.fromOffset(8, 11)
		legiticon.Size = UDim2.fromOffset(29, 16)
		legiticon.Parent = search
		listenProperty(fable.Categories.Main.Object.FableLogo.V4Logo, legiticon, 'ImageColor3', legiticon)
		...
		legiticon.MouseButton1Click:Connect(function()
			clickgui.Visible = false
			fable.Legit.Window.Visible = true
			fable.Legit.Window.Position = UDim2.new(0.5, -350, 0.5, -194)
		end)
	...
	SettingsPane = function(props, children, api)
		...
		local back = Instance.new('ImageButton')
		back.BackgroundTransparency = 1
		back.Image = getfableasset('FableV4/assets/new/backmini.png')
		back.ImageColor3 = color.Light(uipallet.Main, 0.37)
		...
		if props.Main then
			...
			versionlabel.Text = 'Fable '..fable.Version..' '..(
				isfile('FableV4/profiles/commit.txt') and readfile('FableV4/profiles/commit.txt'):sub(1, 6) or ''
			)..' '
		...
		function component:Load(data)
			fable:LoadOptions(self, data)
		end
		
		function component:Save(data)
			data[props.Name] = fable:SaveOptions(self)
		end
		...
		fable.Settings[props.Name] = component
	...
	Slider = function(props, children, api)
		...
		holder.Parent = slider
		local fill = Instance.new('Frame')
		fill.BackgroundColor3 = Color3.fromHSV(fable.GUIColor.Hue, fable.GUIColor.Sat, fable.GUIColor.Value)
		...
		local knob = Instance.new('Frame')
		knob.AnchorPoint = Vector2.new(0.5, 0.5)
		knob.BackgroundColor3 = Color3.fromHSV(fable.GUIColor.Hue, fable.GUIColor.Sat, fable.GUIColor.Value)
		knob.Position = UDim2.fromScale(0.5, 0.5)
		knob.Size = UDim2.fromOffset(14, 14)
		knob.Parent = knobholder
		addCorner(knob, UDim.new(1, 0))
		...
		function component:Color(hue, sat, val, isRainbow)
			fill.BackgroundColor3 = isRainbow and Color3.fromHSV(fable:Color((hue - (self.Index * 0.075)) % 1)) or Color3.fromHSV(hue, sat, val)
	...
	Targets = function(props, children, api)
		...
		local icon = Instance.new('ImageLabel')
		icon.BackgroundTransparency = 1
		icon.Image = getfableasset('FableV4/assets/new/aim.png')
		...
		component.Players = components.TargetsButton({
			...
			Icon = getfableasset('FableV4/assets/new/players.png'),
		...
		component.NPCs = components.TargetsButton({
			...
			Icon = getfableasset('FableV4/assets/new/npcs.png'),
		...
		button.MouseButton1Click:Connect(function()
			...
			holder.BackgroundColor3 = targetswindow.Visible and Color3.fromHSV(fable.GUIColor.Hue, fable.GUIColor.Sat, fable.GUIColor.Value) or color.Light(uipallet.Main, 0.37)
		end)
	...
	TargetsButton = function(props, children, api)
		...
		function component:Toggle()
			...
			tween:Tween(holder, uipallet.Tween, {
				BackgroundColor3 = self.Enabled and Color3.fromHSV(fable.GUIColor.Hue, fable.GUIColor.Sat, fable.GUIColor.Value) or uipallet.Main
			})
		...
		targetsbutton.MouseEnter:Connect(function()
			if not component.Enabled then
				tween:Tween(holder, uipallet.Tween, {
					BackgroundColor3 = Color3.fromHSV(fable.GUIColor.Hue, fable.GUIColor.Sat, fable.GUIColor.Value - 0.25)
				})
	...
	TextList = function(props, children, api)
		...
		local icon = Instance.new('ImageLabel')
		icon.BackgroundTransparency = 1
		icon.Image = getfableasset('FableV4/assets/new/allowediconmini.png')
		...
		textlistwindow.Parent = api.Legit and fable.Legit.Window or clickgui
		component.Window = textlistwindow
		addBlur(textlistwindow)
		addCorner(textlistwindow)
		local icon = Instance.new('ImageLabel')
		icon.BackgroundTransparency = 1
		icon.Image = getfableasset('FableV4/assets/new/allowedicon.png')
		icon.Position = UDim2.fromOffset(10, 13)
		...
		add.Image = getfableasset('FableV4/assets/new/add.png')
		...
		button.MouseButton1Click:Connect(function()
			...
			tween:Cancel(holder)
			holder.BackgroundColor3 = textlistwindow.Visible and Color3.fromHSV(fable.GUIColor.Hue, fable.GUIColor.Sat, fable.GUIColor.Value) or color.Light(uipallet.Main, 0.37)
		end)
		...
		textlist:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
			...
			local actualPosition = (textlist.AbsolutePosition - (api.Legit and fable.Legit.Window.AbsolutePosition or -guiService:GetGuiInset())) / scale.Scale
	...
	TwoSlider = function(props, children, api)
		...
		knobknob.Image = getfableasset('FableV4/assets/new/range.png')
		knobknob.ImageColor3 = Color3.fromHSV(fable.GUIColor.Hue, fable.GUIColor.Sat, fable.GUIColor.Value)
		...
		local arrow = Instance.new('ImageLabel')
		arrow.BackgroundTransparency = 1
		arrow.Image = getfableasset('FableV4/assets/new/rangeindicator.png')
...
fable.Components = setmetatable(components, {
	__newindex = function(_, index, callback)
		...
		if fable.Legit then
			for _, module in fable.Legit.Modules do
				rawset(module, 'Create'..index, function(_, props)
					return callback(props, module.Children, module)
				end)
			end
